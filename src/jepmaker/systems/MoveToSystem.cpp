...
static bool AproximateDestination(Position2D &me, Position2D &to, float aprox)
{
...
exported(void) jepgoSystem(jgo::Game &game, float &time)
{
    ...
    for (std::size_t i = 0; i < move.size(); i++) {
        if (move[i].has_value() && pos[i].has_value()) {
            ...
            game.ecs.emplaceComp(i, Move(Position2D(tmp[0] * move[i].value().getSpeed(), tmp[1] * move[i].value().getSpeed())));
        }
        if (move[i].has_value() && pos[i].has_value()) {
